    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        ...
        <div className="overflow-x-auto">
          <div className="flex items-center gap-1.5 bg-white/5 border border-white/10 rounded-lg px-2 py-1.5 w-max">
          ...
          <div className="divide-y divide-white/4">
            {filtrados.map((pedido) => (
              <div
                ...
                className="flex flex-col gap-1.5 px-4 py-3.5 cursor-pointer hover:bg-violet-500/5 active:bg-violet-500/10 transition-colors"
              >
                ...
                <div className="flex items-center justify-between gap-2">
                  <p className="text-xs text-slate-500">Entrega: {formatDate(pedido.fecha_entrega)}</p>
                  <span className="text-sm font-semibold text-white">{formatCurrency(pedido.total)}</span>
                </div>
              </div>
            ))}
          </div>
              ...
              {filtrados.map((pedido) => (
                ...
                >
                  <td className="px-5 py-3.5">
                    <span className="text-sm font-mono font-semibold text-violet-400 group-hover:text-violet-300 whitespace-nowrap">
                      {pedido.codigo}
                    </span>
                  </td>
                  <td className="px-5 py-3.5">
                    <p className="text-sm font-medium text-slate-100">{pedido.cliente_nombre}</p>
                    <p className="text-xs text-slate-500">{pedido.empleado_nombre}</p>
                  </td>
                  <td className="px-5 py-3.5 text-sm text-slate-400 whitespace-nowrap">{formatDate(pedido.fecha_ingreso)}</td>
                  <td className="px-5 py-3.5 text-sm text-slate-400 whitespace-nowrap">{formatDate(pedido.fecha_entrega)}</td>
