    ...
    <div className="space-y-7">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">Resumen de actividad y estadísticas del negocio</p>
        </div>
          ...
          <CardBody className="pt-1 pb-5">
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={ingresos_semana} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="total" stroke="#8B5CF6" strokeWidth={2.5} fill="url(#gradViolet2)" dot={{ r: 4, fill: "#8B5CF6", strokeWidth: 0 }} activeDot={{ r: 5 }} />
              </AreaChart>
            </ResponsiveContainer>
          </CardBody>
        </Card>
                ...
                <div key={e.estado} className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2">
                    <div
                      ...
                      style={{ background: ESTADO_COLORS[e.estado] ?? PIE_COLORS[i % PIE_COLORS.length] }}
                    />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <CardBody className="pt-1 pb-5">
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={ingresos_mes} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                <defs>
                  <linearGradient id="gradVioletBar" x1="0" y1="0" x2="0" y2="1">
                ...
                <YAxis tick={{ fontSize: 11, fill: "#64748B" }} axisLine={false} tickLine={false} tickFormatter={(v) => `S/${v}`} />
                <Tooltip content={<CustomTooltipMoney />} />
                <Bar dataKey="total" fill="url(#gradVioletBar)" radius={[5, 5, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </CardBody>
        </Card>
        ...
        <Card>
          <CardHeader>
            <h2 className="text-sm font-semibold text-slate-100">Top servicios solicitados</h2>
          </CardHeader>
          <CardBody className="pt-1 pb-5">
            {top_servicios.length === 0 ? (
              <p className="text-sm text-slate-500 text-center py-8">Sin datos aún</p>
            ) : (
                  ...
                  return (
                    <div key={s.nombre}>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs text-slate-300 font-medium truncate">{s.nombre}</span>
                        <span className="text-xs font-bold text-slate-200 ml-2">{s.count}</span>
                      </div>
